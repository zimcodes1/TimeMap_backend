...
## Day 1 — Institution & Organizational Hierarchy

**Why this comes before anything else:** every model built from here forward — venues, courses, timetable entries, admin officers — hangs off department, faculty, school, and ultimately the institution itself. Getting this structure wrong on Day 1 means every later model has to be migrated when it's fixed. This is the one piece of the entire system least safe to leave "roughly right for now."

**Multi-tenancy is the framing for the whole day.** The system is meant to be deployable across many institutions without modification, so `Institution` sits above the hierarchy as the tenant root, and every uniqueness rule below it (school, faculty, department, and later course/venue codes) is scoped to a single institution — never global across the platform. Two different institutions can both have a Computer Science department coded `CSC` without conflict.

### Steps

1. **Model `Institution` as the tenant root** — name, a platform-wide unique short code (this is the one identifier that genuinely is global, since it's what distinguishes tenants from each other), and an active/inactive flag for suspending a tenant without deleting their data.
2. **Model the hierarchy as three linked tables beneath it** — `School` (foreign key to Institution), `Faculty` (foreign key to School), `Department` (foreign key to Faculty). Not as string fields on other models, and not as a single flat table with a `type` column — a proper relational hierarchy is what lets later queries walk up and down it cleanly (e.g. "all departments under this faculty," "the faculty this department belongs to").
3. **Denormalize `institution` directly onto `Faculty` and `Department`** rather than only on `School`. Every tenant-scoped query in the system will need to filter by institution constantly, and requiring a multi-table join up the hierarchy just to check tenant on every request is unnecessary overhead — a direct indexed field, kept in sync from the parent at creation time, is cheap to maintain and expensive to redo everywhere otherwise.
4. **Enforce code uniqueness within the institution, not globally, and not just within the immediate parent.** Department codes specifically need this scoped to the whole institution (not just their faculty) since they double up inside course codes referenced standalone (`CSC301` has no faculty qualifier attached) — two departments in the same institution sharing a code would make those course codes ambiguous.
5. **Build admin-facing creation, not a bulk import script.** Rather than sourcing and seeding a pre-existing institutional structure from an external document, build the actual CRUD flow an admin officer will use in production: add a school, add a faculty under it, add a department under that, with the abbreviation entered directly by the officer and validated for uniqueness within their own institution in real time — surfacing a clear conflict message if the code is already taken, rather than failing silently or crashing. This keeps onboarding self-service for any institution that adopts the platform later, rather than treating NSUK as a special hardcoded case.
6. **Set format guardrails on the `code` field as input validation**, not as an external convention document: canonical uppercase storage with case-insensitive lookup, alphanumeric only, and a sensible maximum length — enforced by the form/API layer, cheap to build, and enough to prevent visually inconsistent entries without adding any process overhead.
7. **Write a handful of sanity queries** confirming the hierarchy is walkable in both directions within a given institution (given a department, get its faculty, school, and institution; given a school, get every department under it) and correctly isolated across institutions (a second test institution's data never appears in the first institution's queries). This isn't a formal test suite yet, just a confidence check before building on top of it.

**End of Day 1 checkpoint:** the `Institution` → `School` → `Faculty` → `Department` structure exists in the database, admin-creatable through real CRUD (not a one-off import), with code uniqueness correctly scoped to each institution and verified isolated between at least two test institutions.
...
6. **Denormalize `institution` onto each user profile model** (`Student`, `LecturerStaff`, `AdminOfficer`), derived from whichever department/faculty/school reference is set. This keeps every tenant-scoped query a direct field check rather than a join through the hierarchy, consistent with the same pattern applied to the hierarchy tables on Day 1.
7. **Decide the identifier convention** per user type — matric number for students, staff ID for lecturers and admins — scoped unique within institution rather than globally, and confirm neither can collide with the other across types within the same institution, since the auth layer built tomorrow will need a single lookup path.
...
1. **Set up JWT issuing** — a login endpoint accepting an identifier (matric number or staff ID) and password, returning an access token and a refresh token on success. Since identifiers are only unique *within* an institution (not platform-wide, per the multi-tenant design), the login flow needs institution context alongside the identifier — resolved either by the client (e.g. a per-deployment subdomain or a bundled institution selector) or an explicit institution field on the login form. Decide this now rather than assuming a bare identifier is enough to resolve a user, since it will be for a single-tenant test but silently break the moment a second institution's data exists in the same database.
