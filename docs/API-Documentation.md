...
_(If `registered_candidates_count` is omitted, it is automatically calculated from live student `CourseRegistration` records)._
...
## 14. Automated Timetable Generation Engine (`/api/scheduling/generate/`)

The Timetable Generation engine replaces manual, entry-by-entry timetable input with an automated Genetic Algorithm scheduler operating over 24 academic 2-hour slots. Detailed architecture and inner workings are documented in [`docs/Timetable-Optimizer-Architecture-and-Integration.md`](Timetable-Optimizer-Architecture-and-Integration.md).

### 14.1 Generate Timetable (Run Genetic Algorithm)

- **URL**: `/api/scheduling/generate/`
- **Method**: `POST`
- **Auth Required**: Yes (Bearer Token, `IsPasswordResetDone`, `CanGenerateTimetable`)
- **Permissions**:
  - Superusers and School Admins can generate for their school and any subordinate faculties or departments.
  - Faculty and Department Admins can only generate if `GenerationScopePermission` has enabled `allow_faculty_generation` or `allow_department_generation`.

#### Request Payload

| Field                      | Type    | Required | Default | Description                                                               |
| -------------------------- | ------- | -------- | ------- | ------------------------------------------------------------------------- |
| `semester_id` / `semester` | Integer | Yes      | —       | ID of target `Semester`. Both keys are accepted.                          |
| `scope_type`               | String  | Yes      | —       | `"school"`, `"faculty"`, or `"department"`.                               |
| `scope_id`                 | Integer | Yes      | —       | Target School, Faculty, or Department ID.                                 |
| `population_size`          | Integer | No       | 60      | Candidate chromosomes per generation (10–300).                            |
| `max_generations`          | Integer | No       | 150     | Maximum GA iterations before stopping (10–1000).                          |
| `mutation_rate`            | Float   | No       | 0.08    | Gene mutation probability (0.01–1.0).                                     |
| `stagnation_limit`         | Integer | No       | 40      | Stagnation patience limit before returning Best Available (5–200).        |
| `publish_immediately`      | Boolean | No       | false   | If true, automatically publishes the run to live entries upon completion. |
...
```json
{
	...
	"completed_at": "2026-09-15T02:00:01Z"
}
```

---

### 14.2 List Past Generation Runs

- **URL**: `/api/scheduling/generate/runs/`
- **Method**: `GET`
- **Auth Required**: Yes
- **Query Parameters**: `semester` (ID), `scope_type` (`"school" | "faculty" | "department"`), `scope_id` (ID).

---

### 14.3 Retrieve Run Detail & Conflict Diagnostics

- **URL**: `/api/scheduling/generate/runs/{id}/`
- **Method**: `GET`
- **Auth Required**: Yes

Returns complete run diagnostics, fitness, conflict breakdown, and assignments payload.

---

### 14.4 Publish Generated Run to Live Timetable

- **URL**: `/api/scheduling/generate/runs/{id}/publish/`
- **Method**: `POST`
- **Auth Required**: Yes (`CanGenerateTimetable`)

Performs an atomic database transaction:

1. Deletes previous scoped `TimetableEntry` records for the semester.
2. Creates recurring `TimetableEntry` records with `RRULE:FREQ=WEEKLY;BYDAY=...`.
3. Materializes date-stamped `LectureSession` instances across active lecture weeks.
4. Marks the generation run as `is_published = true`.
...
### 14.5 Scope Generation Permissions Policy

- **URL**: `/api/scheduling/generate/permissions/`
- **Method**: `GET`, `PATCH`
- **Auth Required**: Yes (`GET`: All authenticated; `PATCH`: Superuser only)

#### GET Query Parameters

- `school`: School ID (defaults to requesting user's school if omitted).
